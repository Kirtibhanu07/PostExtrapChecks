...
def _html_into(placeholder, content: str) -> None:
    ...
    instead of appending to the normal page flow. Used so the banner,
    stage stepper, and log render into positions reserved earlier in the
    script (Step 2/3, above Step 1) even though the code that computes
    their final content runs later, in one single pass per script run —
    not written into from inside run_pipeline() while it's executing.
    """
...
/* Stage ran OK but its own Check column found mismatches — still a
   completed stage (icon/label stay green), but the ring + a small red
   badge make it obvious the output needs review before trusting it. */
.n-stage-col.done.flagged .n-stage-icon { box-shadow: 0 0 0 4px rgba(214,69,90,0.18); }
.n-stage-flag {
    font-size: 10px;
    font-weight: 700;
    color: var(--n-fail);
    background: var(--n-fail-wash);
    border: 1px solid rgba(214,69,90,0.3);
    border-radius: 20px;
    padding: 2px 8px;
    margin-top: 2px;
}

/* Active stage — pulsing ring so the currently-running stage is
   obvious while the pipeline is executing, not only after it finishes. */
@keyframes n-pulse-ring {
    0%   { box-shadow: 0 0 0 0 rgba(110,55,250,0.45); }
    70%  { box-shadow: 0 0 0 10px rgba(110,55,250,0); }
    100% { box-shadow: 0 0 0 0 rgba(110,55,250,0); }
}
.n-stage-col.active .n-stage-icon { animation: n-pulse-ring 1.4s ease-out infinite; }
...
/* Stage completed (would render fully green) but its own Check column
   has mismatches in it — left border switched to red as an unmissable
   "this ran, but don't trust it blindly" signal, without touching the
   ok/fail/skip logic that decided the base card color above. */
.n-result-card.ok.flagged { border-left: 4px solid var(--n-fail); }
...
.n-rc-flag {
    ...
    margin-top: 8px;
    padding-top: 8px;
    border-top: 1px dashed rgba(214,69,90,0.3);
}
...
if "meta_user" not in st.session_state:
    st.session_state.meta_user = ""
if "meta_project" not in st.session_state:
    st.session_state.meta_project = ""
...
def render_stage_stepper_into(ph):
    ...
    for n, title, sub in STAGE_DEFS:
        ...
        if state == "done" and st.session_state.result_stats:
            ...
            flags = s.get("flags")
            if flags and flags.get("failed"):
                word = "FALSE" if flags["kind"] == "bool" else "mismatched"
                flag_html = f'<div class="n-stage-flag">{flags["failed"]} {word}</div>'
        cls = f"{state} flagged" if flag_html else state
        cols_html += f"""
        <div class="n-stage-col {cls}">
...
def check_flags(df: pd.DataFrame) -> dict:
    """
    Read-only inspection of a stage's own 'Check' column — does NOT
    recompute or alter anything qc_check.py produced. Two shapes exist
    in this pipeline's output:

      - Boolean Check (RateCheck, ExposureCheck): True/False per row.
        We count False rows directly.
      - Numeric Check (ExtrapCheck: Msec% - BT%): expected to sit at
        0 when extrapolation matches the Sample-derived split. There
        is no True/False here, so we count rows where Check != 0 as
        "mismatched" rather than "false" — same idea, different label
        so it isn't confused with an actual boolean failure.

    Returns {} if the sheet has no 'Check' column (e.g. Sample), since
    that stage has nothing of this kind to flag.
    """
    if df is None or "Check" not in df.columns:
        return {}
    col = df["Check"]
    non_null = col.dropna()
    if non_null.empty:
    ...
    if non_null.isin([True, False]).all():
        return {
            "kind": "bool",
            "failed": int((non_null == False).sum()),
            "checked": int(len(non_null)),
        }
    else:
        mismatched = int((non_null != 0).sum())
        return {
            "kind": "numeric",
            "failed": mismatched,
            "checked": int(len(non_null)),
...
# ── Excel writer ──────────────────────────────────────────────────────────────
README_ROWS = [
    ("Sheet", "What it compares", "Check column"),
    ("RateCheck", "Adapt pivot (adapt_country, adapt_Rate, cols left of Check) vs BSR+YouTube MM pivot (ref_country, ref_Rate, cols right of Check)", "Check = TRUE/FALSE — TRUE when country+Rate match on both sides"),
    ("ExposureCheck", "Sample pivot (s_matchday/s_brand/s_tool/s_location/s_sequences/s_exposure_msec, left) vs Matex pivot (m_* equivalents, right)", "Check (col H) = TRUE/FALSE — TRUE only when all 6 left values match all 6 right values"),
    ("Sample", "Sample/programs pivoted by pID_MM + matchday + BT_block, plus a separate matchday summary table (matchday/BT_Block/Msec) used by ExtrapCheck", "No Check column on this sheet — it's a lookup source, not a QC comparison"),
    ("ExtrapCheck", "Adapt pivot with BT_pct and Msec_pct, both derived using the Sample summary as the lookup table", "Check = Msec_pct - BT_pct — a NUMBER expected at/near 0, not TRUE/FALSE. Non-zero = mismatch"),
]

def _write_readme_sheet(wb, stats=None):
    """
    Purely additive documentation sheet — does not read from or alter
    any of the 4 QC sheets or their Check values. Safe to skip (stats
    can be None) since it only ever adds context, never a dependency.
    """
    ws = wb.create_sheet(title="ReadMe")
    ws.sheet_properties.tabColor = "6E37FA"
    ...
    if stats:
        flag_row = len(rows) + 3
        ws.cell(row=flag_row, column=1, value="This run's flagged rows").font = Font(bold=True, name="Calibri", size=10)
        r = flag_row + 1
        names = {1: "RateCheck", 2: "ExposureCheck", 3: "Sample", 4: "ExtrapCheck"}
        for n, sname in names.items():
            s = stats.get(n, {})
            flags = s.get("flags")
            if flags and flags.get("failed"):
                word = "FALSE" if flags["kind"] == "bool" else "mismatched (Check != 0)"
                ws.cell(row=r, column=1, value=sname).font = CELL_FONT
                ws.cell(row=r, column=2,
                        value=f'{flags["failed"]} of {flags["checked"]} rows {word}').font = Font(color="CC0000", name="Calibri", size=10, bold=True)
                r += 1
        if r == flag_row + 1:
            ws.cell(row=r, column=1, value="No Check failures/mismatches in this run.").font = Font(color="22A06B", name="Calibri", size=10)
    ...
    for row in ws.iter_rows():
        for c in row:
            if c.row > 1:
                ws.row_dimensions[c.row].height = 34


def write_output_styled(sheets: dict, stats=None) -> bytes:
    wb = Workbook()
    wb.remove(wb.active)

    _write_readme_sheet(wb, stats)
...
def run_pipeline(adapt_f, bsr_f, yt_f, matex_f, sample_f,
                  banner_ph=None, stage_ph=None, log_ph=None,
                  user_name="", project_name=""):
    """
    banner_ph / stage_ph / log_ph are the SAME st.empty() placeholders
    Step 2/3 (and the banner above Step 1) already declare before this
    function is ever called. Passing them in only lets this function
    push a render into a position that already exists on the page —
    it does not change what gets computed, only when the existing
    stage-stepper / log / banner rendering is shown. If a placeholder
    isn't passed (e.g. any future non-UI caller), rendering is simply
    skipped for that one, exactly as before.
    """
    def _live():
        if banner_ph is not None: render_banner_into(banner_ph)
        if stage_ph  is not None: render_stage_stepper_into(stage_ph)
        if log_ph    is not None: render_log_into(log_ph)
    ...
    try:
        ...
        if user_name.strip() or project_name.strip():
            add_log(f"Run by {user_name.strip() or '—'}  ·  Project: {project_name.strip() or '—'}", "dim")
        add_log("", "dim")
        add_log("── LOAD ──────────────────────────────────────────────", "stage")
        ...
        # Stage 1
        set_stage(1, "active"); _live()
        try:
            df = build_rate_check(frames["adapt"], frames["bsr"], frames["yt"])
            output_sheets["RateCheck"] = df
            flags = check_flags(df)
            stats[1] = {"rows": len(df), "ok": True, **({"flags": flags} if flags else {})}
            set_stage(1, "done")
            if flags and flags["failed"] > 0:
                add_log(f"  [✓]  RateCheck        {len(df):,} rows", "good")
                add_log(f"       ⚠ {flags['failed']} of {flags['checked']} Check rows = FALSE (country+rate mismatch)", "warn")
            else:
                add_log(f"  [✓]  RateCheck        {len(df):,} rows", "good")
        except Exception as e:
        ...
        # Stage 2
        set_stage(2, "active"); _live()
        try:
            df = build_exposure_check(frames["sample"], frames["matex"])
            output_sheets["ExposureCheck"] = df
            flags = check_flags(df)
            stats[2] = {"rows": len(df), "ok": True, **({"flags": flags} if flags else {})}
            set_stage(2, "done")
            add_log(f"  [✓]  ExposureCheck    {len(df):,} rows", "good")
            if flags and flags["failed"] > 0:
                add_log(f"       ⚠ {flags['failed']} of {flags['checked']} Check rows = FALSE (Sample vs Matex mismatch)", "warn")
        except Exception as e:
        ...
        try:
            ...
            flags = check_flags(df)
            stats[4] = {"rows": len(df), "ok": True, **({"flags": flags} if flags else {})}
            set_stage(4, "done")
            add_log(f"  [✓]  ExtrapCheck      {len(df):,} rows", "good")
            if flags and flags["failed"] > 0:
                add_log(f"       ⚠ {flags['failed']} of {flags['checked']} rows have Msec%-BT% ≠ 0", "warn")
        ...
        if output_sheets:
            try:
                st.session_state.output_bytes = write_output_styled(output_sheets, stats)
                add_log(f"  [✓]  {done}/4 sheets written → QC_Output.xlsx", "good")
            except Exception as e:
                add_log(f"  [✗]  Write failed: {e}", "fail")

        add_log("", "dim")
        flagged_stages = [n for n, s in stats.items() if s.get("ok") and s.get("flags", {}).get("failed")]
        if done == 4 and not flagged_stages:
            add_log("  ✓  COMPLETE — all 4 sheets written", "good")
        elif done == 4:
            add_log(f"  ⚠  COMPLETE, BUT {len(flagged_stages)} sheet(s) have Check mismatches — review before trusting output", "warn")
        elif done > 0:
        ...
        # Runs on every exit path — normal completion, the early return
        # on load failure, or any exception that somehow escapes the
        # try/excepts above — so pipeline_running can never get stuck
        # True and the history entry is never skipped or duplicated
        # regardless of how the run ended. The actual re-render of the
        # page happens once, after this function returns, via the
        # normal top-to-bottom script flow and st.rerun() — not from in
        # here — see the note above render_stage_stepper_into for why.
        st.session_state.pipeline_running = False
        ...
        if st.session_state.result_stats is not None:
            ...
            st.session_state.run_history.insert(0, {
                ...
                "user": user_name.strip() or "—",
                "project": project_name.strip() or "—",
            })
...
with meta_col1:
    st.session_state.meta_user = st.text_input(
        "Run by", value=st.session_state.meta_user,
        placeholder="Your name", key="meta_user_input",
    )
with meta_col2:
    st.session_state.meta_project = st.text_input(
        "Project", value=st.session_state.meta_project,
        placeholder="Project name", key="meta_project_input",
    )
...
with action_progress_col:
    ...
    """)
with action_btn_col:
    # The click is captured here, but run_pipeline() runs later in the
    # script, after stage_ph/log_ph are declared below — that's what
    # lets the single post-run render land in Step 2/3's actual
    # position on the page instead of appearing wherever this line is.
...
# ═══════════════════════════════════════════════════════════════════════════
# Run the pipeline now, if the button was clicked. banner_ph / stage_ph /
# log_ph are the same three placeholders declared above (banner before
# Step 1, stage_ph/log_ph in Step 2/3) — run_pipeline() now pushes a
# render into them after each stage transition, so the currently-active
# stage and running log are visible WHILE the pipeline executes, not
# only in the single final pass below. That final pass still runs on
# every script execution (page load, post-run rerun, or any other
# rerun e.g. toggling "Show logs") so the end state is always correct
# regardless of what triggered it — the live updates during the run
...
if run_clicked:
    with st.spinner("Running QC pipeline…"):
        run_pipeline(
            ...
            banner_ph=banner_ph, stage_ph=stage_ph, log_ph=log_ph,
            user_name=st.session_state.meta_user,
            project_name=st.session_state.meta_project,
        )
    st.rerun()

# Single render pass, using whatever session_state currently holds —
# covers page load, the rerun right after a pipeline run, and reruns
# triggered by anything else (e.g. toggling "Show logs").
...
if st.session_state.result_stats:
    ...
    with st.container(border=True, key="output_results"):
        ...
        with st.expander("How to read these results", expanded=False):
            st.markdown("""
**RateCheck** — compares the Adapt pivot (`adapt_country`, `adapt_Rate` …, left half) against the combined BSR + YouTube MM pivot (`ref_country`, `ref_Rate` …, right half). The **`Check`** column, sitting between the two halves, is `TRUE` when `country+Rate` matches on both sides for that row, `FALSE` when it doesn't.

**ExposureCheck** — compares the Sample pivot (`s_matchday`, `s_brand`, `s_tool`, `s_location`, `s_sequences`, `s_exposure_msec`, left half) against the Matex pivot (`m_matchday`, `m_brand`, `m_tool`, `m_location`, `m_sequences`, `m_exposure_msec`, right half). The **`Check`** column (col H) is `TRUE` only when all six left values match all six right values for that row.

**Sample** — a pivot of Sample/programs by `pID_MM` · `matchday` · `BT_block` with summed exposure, plus a separate matchday summary table (`matchday`, `BT_Block`, `Msec`) alongside it. This sheet has **no `Check` column** — it's a lookup source for ExtrapCheck, not a QC comparison, so nothing here gets flagged as pass/fail.

**ExtrapCheck** — the Adapt pivot with two derived columns, `BT_pct` (row exposure ÷ that matchday's total BT_Block) and `Msec_pct` (row exposure ÷ that matchday's total Msec), both looked up from the Sample summary. The **`Check`** column is `Msec_pct − BT_pct`, a number expected to sit at (or very near) **0** when the extrapolation lines up — it is not a `TRUE`/`FALSE` flag like the other two sheets. Rows where `Check ≠ 0` are counted as mismatched below.

Any stage card with a red flag below its row count found `Check` failures/mismatches in its own sheet — the stage still *ran* successfully (hence the green ✓), but its output has rows that need review before you trust them.
            """)
        ...
        for i, sname in enumerate(names, 1):
            ...
            if ok:
                ...
                # Stage completed (card would otherwise render fully
                # green) — but its own Check column has mismatches.
                # Surface that explicitly rather than showing plain
                # green and burying it in the log.
                flags = s.get("flags")
                if flags and flags.get("failed"):
                    cls += " flagged"
                    word = "rows = FALSE" if flags["kind"] == "bool" else "rows mismatched (Check ≠ 0)"
                    flag_html = (
                        f'<div class="n-rc-flag">⚠ {flags["failed"]} of {flags["checked"]} {word}</div>'
                    )
            elif skipped:
...
if st.session_state.run_history:
    ...
    with st.container(key="run_history", border=True):
        for idx, run in enumerate(st.session_state.run_history):
            ...
            # .get() with a fallback — older entries created before the
            # user/project fields existed won't have these keys, and
            # this must not KeyError on them.
            run_user    = html_lib.escape(run.get("user", "—"))
            run_project = html_lib.escape(run.get("project", "—"))
                ...
                <div class="n-history-row" style="border-bottom:none; padding-bottom:0;">
                  <div class="n-history-left">
                    ...
                    <div>
                      ...
                      <div class="n-history-status">{dtext}  ·  {run_user}  ·  {run_project}</div>
                    </div>
                  </div>
                </div>
                """)
