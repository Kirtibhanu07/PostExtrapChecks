...
def _html(content: str) -> None:
    ...
    Streamlit's markdown renderer treats any block indented 4+ spaces
    (after a blank line) as a fenced code block, per standard Markdown
    rules. Every f-string HTML block written inside a Python for/if/with
    statement inherits that block's indentation, so without this,
    nested HTML renders as literal grey monospace text instead of the
    intended styled component. textwrap.dedent() strips the common
    leading whitespace before handing it to st.markdown, regardless of
    how deeply the call is nested in Python.
    """
...
/* Browse-files button: visible, on-brand, never scaled to invisibility */
...
.n-upload-card [data-testid="stBaseButton-secondary"] {
    ...
    color: var(--n-violet) !important;
    border: 1px solid var(--n-violet) !important;
...
.n-upload-card [data-testid="stFileUploader"] button:hover,
.n-upload-card [data-testid="stFileUploaderDropzone"] button:hover {
